...
def process_files_optimized(folder_path):
    ...
    files = [f for f in os.listdir(folder_path) 
             if f.startswith("_Book2_RU_") and f.endswith("_vocab.txt")]
...
def save_readable_json(data, output_filename="_book2_note_v.json"):
...
def save_compact_but_readable(data, output_filename="vocab_compact.json"):
...
def main():
    ...
    if data:
        ...
        save_readable_json(data, "_book2_note_v.json")
        
        # 2. Более компактная но все еще читаемая версия
        save_compact_but_readable(data, "vocab_compact.json")
        
        # 3. Минимальная версия для сравнения
        with open("vocab_minimal.json", 'w', encoding='utf-8') as f:
        ...
        for filename in ["_book2_note_v.json", "vocab_compact.json", "vocab_minimal.json"]:
        ...
        print("="*50)
        print("""
1. Для Anki используйте 'vocab_compact.json' - он читаемый и не слишком большой
...
3. В Anki загружайте через:
   import json
   with open('vocab_compact.json', 'r', encoding='utf-8') as f:
       data = json.load(f)
