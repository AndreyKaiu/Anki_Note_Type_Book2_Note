...
anki_card_model_V = genanki.Model(
    ...
    lookupWordAllLangs(index).then(allLanguagesData => {
        ...
        languages.forEach(lang => {
            ...
            if (entry) {
                ...
                const soundName = 'book2_voc_N' + index + '_' + lang + '.mp3';
...
.showWordSlowly::after {
    animation: unmaskR
        var(--ws-duration, 1s)  /* По умолчанию 1s */
        linear 
        forwards
        var(--ws-delay, 0s);      /* По умолчанию 0s */
}
...
/* заливка (от min до value!) */
.myRange::-moz-range-progress {
    height: 6px;
    background: #7f97b0; /* ← мягкий цвет */
    border-radius: 3px;
}
